...
# 给表格模型输出多加一个字段 cell_ocr_items，用来保存每个单元格里的 OCR 块。以便可以把单元格内的可能粘连的文本块提前区分出来
@dataclass
class WiredTableOutput:
    pred_html: Optional[str] = None
    cell_bboxes: Optional[np.ndarray] = None
    logic_points: Optional[np.ndarray] = None
    elapse: Optional[float] = None
    cell_ocr_items: Optional[List[Dict[str, any]]] = None
...
class WiredTableRecognition:
    ...
    def __call__(
        ...
        **kwargs,
        ...
        try:
            ...
            # 在生成 HTML 前后保留 cell_ocr_items，以便后续分析单元格内的 OCR 结果
            # 在 HTML 拼接前，把单元格内的 OCR 块先保存下来，避免后面 HTML 把它们粘成一个字符串。
            cell_ocr_items = self.build_cell_ocr_items(t_rec_ocr_list)
            t_rec_ocr_list = self.sort_and_gather_ocr_res(t_rec_ocr_list)
        ...
        # 在 HTML 拼接前，把单元格内的 OCR 块先保存下来，避免后面 HTML 把它们粘成一个字符串。
        return WiredTableOutput(pred_html, polygons, logi_points, elapse, cell_ocr_items)
    ...
    """
        作用：
        把 MinerU 内部的 t_rec_ocr_list 转成更干净的 JSON 结构。
        ...
        ocr_items
    """
    def build_cell_ocr_items(self, t_rec_ocr_list: List[Dict[str, any]]) -> List[Dict[str, any]]:
        cell_items = []
        for cell_idx, cell in enumerate(t_rec_ocr_list):
            ocr_items = []
            for ocr_idx, ocr_item in enumerate(cell.get("t_ocr_res", [])):
                if len(ocr_item) < 2:
                    continue
                ...
                ocr_items.append({
                    "ocr_id": f"c{cell_idx}_o{ocr_idx}",
            ...
            cell_items.append({
                ...
                "cell_index": cell_idx,
                "cell_bbox": [int(v) for v in cell.get("t_box", [])],
                "logic_box": [int(v) for v in cell.get("t_logic_box", [])],
                "ocr_items": ocr_items,
            })
...
class UnetTableModel:
    ...
    def predict(self, input_img, ocr_result, wireless_html_code, return_metadata: bool = False):
        ...
        try:
            ...
            """
            当调用 predict(..., return_metadata=True) 时，除了 HTML，也能拿到：
            wired_cell_ocr_items也就是我们真正想要的“单元格 + OCR 块”。
            """
            ...
            if return_metadata:
                return {
                    ...
                    "wired_logic_points": None if wired_structure_results is None else wired_structure_results.logic_points,
                    "wired_html": wired_html_code,
                    "wired_cell_ocr_items": wired_table_results.cell_ocr_items,
                }
