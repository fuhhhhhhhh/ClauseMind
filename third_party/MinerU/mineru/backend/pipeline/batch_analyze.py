        ...
        if self.table_enable:
            ...
            if wired_table_res_list:
                for table_res_dict in tqdm(
                    ...
                    # 把 wired_cell_ocr_items 写进 MinerU 最后的 *_model.json / *_middle.json 这类输出结构里。
                    wired_table_metadata = wired_table_model.predict(
                        table_res_dict["wired_table_img"],
                        table_res_dict["ocr_result"],
                        table_res_dict["table_res"].get("html", None),
                    ...
                    if isinstance(wired_table_metadata, dict):
                        table_res_dict["table_res"]["html"] = wired_table_metadata.get("html", "")
                        table_res_dict["table_res"]["wired_cell_ocr_items"] = wired_table_metadata.get("wired_cell_ocr_items", [])
                        table_res_dict["table_res"]["selected_model"] = wired_table_metadata.get("selected_model", "")
                    else:
                        table_res_dict["table_res"]["html"] = wired_table_metadata
